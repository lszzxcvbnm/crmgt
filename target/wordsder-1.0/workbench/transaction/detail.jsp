<%@ page import="com.lsz.crm.settings.domain.DicValue" %>
<%@ page import="java.util.List" %>
<%@ page import="java.util.Map" %>
<%@ page import="java.util.Set" %>
<%@ page import="com.lsz.crm.workbench.domain.Tran" %>
...
//准备阶段和可能性之间的对应关系
   ...
   //根据pMap准备pMap在的key集合
	...
	for(int i=0;i<dvList.size();i++){
		...
		//如果可能性为0，说明找到了前面正常阶段和后面丢失极端的分界点
...
<script type="text/javascript">
	...
	function showHistoryList(){
		$.ajax({
			url:"workbench/transaction/getHistoryListByTranId.dao",
			data:{
				"tranId":"${t.id}"
			},
			type:"get",
	...
	* */
	function changeStage(stage,i){
		...
		$.ajax({
			url:"workbench/transaction/changeStage.dao",
			data: {
				"id":"${t.id}",
				"stage":stage,
			...
			success:function (data){
                 if(data.success){
                   //成功后需要在详细信息页上局部刷新，刷新阶段，可能性，修改人，修改时间
	...
	/*	alert(currentStage);
		...
		alert(index);
		alert(point);*/
		//如果当前阶段的可能性为0 前7个一定是黑圈，一个红叉，一个黑叉
		if(currentPossibility=="0"){
			...
			//如果当前阶段的可能性不为0 前7个一定是琭圈，琭色标记，黑圈，后两个一定是黑叉
		}else{
			//遍历前7个
			for(var i=0;i<point;i++){
				//如果是当前阶段
...
<body>
	...
	<div style="position: relative; left: 40px; top: -50px;">
		...
		//判断当前阶段
			//如果当前阶段的可能行为0 前7个一定是黑圈，后两个一个是红叉，一个是黑叉
		...
		//如果遍历出来的阶段性的可能性不为0。7个有可能是绿圈，绿色标记，黑圈
					}else{
                          //如果是当前阶段
						if(i==index){
                             //绿色标记
	...
	<div style="position: relative; top: 0px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 30px;">
			...
			<div style="width: 300px;position: relative; left: 650px; top: -60px;"><b id="possibility">${t.possibility}</bid></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px;"></div>
			<div style="height: 1px; width: 400px; background: #D5D5D5; position: relative; top: -60px; left: 450px;"></div>
		</div>
