...
<script type="text/javascript">
	...
	//	为保存按钮绑定事件，执行备注添加事件操作
		$("#saveRemarkBtn").click(function(){
             $.ajax({
				url:"workbench/activity/saveRemark.dao",
				 data:{
        ...
        //为备注修改的更新操作绑定事件
            $("#updateRemarkBtn").click(function(){
                var id=$("#remarkId").val();
                    $.ajax({
                        url:"workbench/activity/updateRemark.dao",
                        data:{
                               "id":id,
	...
	function showRemarkList(){
		$.ajax({
			url:"workbench/activity/getRemarkListByAid.dao",
			data:{
                   "activityId":"${a.id}"
			},
			type:"get",
    ...
    function editRemark(id){
       ...
       //将h5中的消息，赋予到修改操作模态窗口的文本域中
       $("#noteContent").val(noteContent);

       //以上消息处理完毕后，将修改备注的模态窗口打开
	...
	function deleteRemark(id){
            $.ajax({
				url:"workbench/activity/deleteRemark.dao",
				data: {
					"id":id
				},
				type:"post",
				...
				success:function(data){
                      if(data.success){
                           //删除备注n
						  //showRemarkList();使用 的是before,会记录原数据保留
						 // showRemarkList();
						 $("#"+id).remove();
					  }else{
                      	alert("删除备注失败");
...
<body>
	...
	<div style="position: relative; top: -70px;">
		...
		<div style="position: relative; left: 40px; height: 30px; top: 30px;">
			...
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${a.createBy}&nbsp&nbsp&nbsp&nbsp;</b><small style="font-size: 10px; color: gray;">${a.createTime}</small></div>
			<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
		<div style="position: relative; left: 40px; height: 30px; top: 40px;">
			<div style="width: 300px; color: gray;">修改者</div>
			<div style="width: 500px;position: relative; left: 200px; top: -20px;"><b>${a.editBy}&nbsp;&nbsp&nbsp&nbsp&nbsp;</b><small style="font-size: 10px; color: gray;">${a.editTime}</small></div>
			<div style="height: 1px; width: 550px; background: #D5D5D5; position: relative; top: -20px;"></div>
		</div>
