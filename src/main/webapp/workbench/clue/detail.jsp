...
<script type="text/javascript">
	...
	//	页面加载完毕后，取出关联的市场活动信息列表
		showActivityList();
	//	为关联市场活动模态窗口中的搜索框绑定事件,通过回车键来触发查询，并展现活动列表

		$("#aname").keydown(function(event){
			if(event.keyCode==13){
				// alert ("展现并关联市场活动表");
				$.ajax({
					url: "workbench/clue/getActivityListByNameNotById.dao",
					data: {
		...
		$("#bundBtn").click(function(){
			...
			if($xz.length==0){
				...
				//workbench/clue/bund.dao?clueId=xxx&&aid==xx...
				var param="cid=${c.id}&";
	...
	function showActivityList(){
		$.ajax({
			url:"workbench/clue/getActivityListByClueId.dao",
			data:{
			"clueId":"${c.id}"
			},
			type:"get",
