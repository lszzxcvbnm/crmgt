...
<script>
	...
	* .....
	* 数据量少，写进表没多大意义
	* 需要用到properties属性文件来保存
	* stage2Properties.properties
	*  01资质审查=10
	*  02需求分析=25
	* ....
	* 解析在服务器缓存中
	...
	$(function(){
		$("#create-customerName").typeahead({
			source: function (query, process) {
				$.get(
						"workbench/transaction/getCustomerName.dao",
						{ "name" : query },
						function (data) {
							//alert(data);
							process(data);
		...
		$(".time2").datetimepicker({
           ...
           //为阶段的下拉框，绑定选中下拉的事件，填写可能性
       $("#create-stage").change(function(){
                //取得选中的阶段
			var stage=$("#create-stage").val();
			/*
			目标：填写可能性，
			阶段和可能性之间的对应关系pMap，但是pMap是Java 语言中的键值队关系
			将pMap转换为json
			现在不能以json.key的形式来取value因为stage是可变的变量就不能用传统的json.key的形式来取值
			现在取值用json[key]
			 */
...
<body>
	...
	<form action="workbench/transaction/save.dao" id="tranForm" method="post"  class="form-horizontal" role="form" style="position: relative; top: -30px;">
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				<select class="form-control" id="create-clueSource" name="source">
				  <option></option>
					<c:forEach items="${sourceList}" var="r">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" value="589de87ab3ff4bad90395a3629ee0883" name="activityId"/>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-sm-10" style="width: 300px;">
				...
				<input type="hidden" value="546f139cc24541a9ae47c66e1ab6d38d" name="contactsId"/>
			</div>
		</div>
